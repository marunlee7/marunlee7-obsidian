...
- 현우 영일만 친구 - USB 체크
- [ ] 플러그인 note auto mover 설정
- [ ] 태그 작가 템플릿 수정, 이미지 넣기 추가 [yes24 작가파일, 벅스 참조,]
## 오늘 기억할 일
...
## 오늘 작성한 노트
```dataview
List FROM "" WHERE file.cday = date("2025-10-10") SORT file.ctime desc
limit 5
```
## 오늘 수정한 노트
```dataview
List FROM "" WHERE file.mday = date("2025-10-10") SORT file.mtime desc
limit 5
```