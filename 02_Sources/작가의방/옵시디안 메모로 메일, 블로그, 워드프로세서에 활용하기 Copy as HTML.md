---
aliases: []
source_name: 옵시디안 메모로 메일, 블로그, 워드프로세서에 활용하기 Copy as HTML
references: https://www.youtube.com/watch?v=CI17UN4IzGo&list=PLy4SLsxzyLUUqeeQzFyXoS750M3_km_pq&index=11
links: 
---
# 옵시디안 메모로 메일, 블로그, 워드프로세서에 활용하기 Copy as HTML

![](https://www.youtube.com/watch?v=CI17UN4IzGo)  

옵시디안에 빠져들면 빠져들수록 다양한 앱과 함께 쓰게 되는데요. 아직 마크다운을 기본으로 지원하는 앱이 별로 없어서 옵시디안 메모를 다른 앱에 복사해넣으면 조금 이상하게 붙여넣기 되는 경우가 많습니다. 그런데 아주 간단플러그인 하나만 설치하면 이제, 워드든, 이메일이든, 블로그든 옵시디안 메모 스타일을 유지하면서 콘텐츠를 이쁘게 복사해넣을 수 있습니다. 활용 방법이 무궁무진한 새 플러그인을 소개해드립니다.  

00:00 하일라이트  
00:15 오늘의 목표  
00:46 플러그인 설치와 설정  
03:36 사용 방법  
05:42 활용 예시  
08:16 설정 추가 설명  
08:55 마무리  

\----------------  
작가님들~, 어서오세요! 여기는 작가의 방입니다.  

너무나 매력적인 메모앱 옵시디안(Obsidian)을 배워봅니다.  
제텔카스텐 방식을 제대로 구현한 강력한 무료 앱입니다.  
글을 쓰는 우리 작가님들의 제2의 뇌가 되어줄 메모앱을 소개합니다.  
작가님들, 옵시디안을 처음 접했거나, 앞으로 쓸 계획이 없더라도 한번 꼭 보시길 강추 합니다!  


## 要約 ✍︎

## 思索 🙇🏻
