---
aliases: []
source_name: 옵시디안 여러 파일명을 한꺼번에 바꾸기 Bulk Rename
references: https://www.youtube.com/watch?v=IZgCl_p78sM&list=PLy4SLsxzyLUUqeeQzFyXoS750M3_km_pq&index=12
links: 
---
# 옵시디안 여러 파일명을 한꺼번에 바꾸기 Bulk Rename

![](https://www.youtube.com/watch?v=IZgCl_p78sM)  

작가님들~어서 오세요! 여기는 작가의 방입니다. 오늘은 계속 늘어나는 메모를 편리하게 관리할 수 있는 툴을 하나 소개해드리겠습니다. 메모가 쌓이다보면 메모 제목을 일괄로 변경하고 싶을 때가 있는데요, 이런 기능을 옵시디안이 기본으로 내장하고 있지 않습니다. 하지만 오늘 소개해드리는 간단한 플러그인 사용하면, 좀더 쉽게 가능합니다.  

00:00 하일라이트  
00:15 오늘의 목표  
01:13 플러그인 개요  
01:52 설치  
02:31 기본 사용법  
05:58 응용\_폴더 바꾸기  
07:16 파일명 변경과 문서 내 링크 변경 테스트  
08:48 총평  

\----------------  
작가님들~, 어서오세요! 여기는 작가의 방입니다.  

너무나 매력적인 메모앱 옵시디안(Obsidian)을 배워봅니다.  
제텔카스텐 방식을 제대로 구현한 강력한 무료 앱입니다.  
글을 쓰는 우리 작가님들의 제2의 뇌가 되어줄 메모앱을 소개합니다.  
작가님들, 옵시디안을 처음 접했거나, 앞으로 쓸 계획이 없더라도 한번 꼭 보시길 강추 합니다!  

## 要約 ✍︎

## 思索 🙇🏻
