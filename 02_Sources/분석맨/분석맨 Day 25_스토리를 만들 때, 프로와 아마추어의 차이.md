---
title: 분석맨 Day 25_스토리를 만들 때, 프로와 아마추어의 차이
aliases: []
level: T02
file_role: article_atom
parent: "분석맨"
word_type: t3전용 # T03 전용 필드 (빈 값 유지)
status: planned
priority: medium
process: raw
source_name: 분석맨 Day 25_스토리를 만들 때, 프로와 아마추어의 차이
source_author: "분석맨"
source_type: "web"  
references:
tags: [출처/외부_정보/아티클_웹]
---
# 분석맨 Day 25_스토리를 만들 때, 프로와 아마추어의 차이




## My Thoughts

>모든 초고는 쓰레기다.
>어니스트 헤밍웨이
>The first draft of anything is shit. Earnest Hemingway

창작동화자, 작가가 가져야 할 마인드에 관한 영상이다.
핵심은 위의 헤밍웨이의 명언으로 요약할 수 있다.
하지만, 정착 이를 실천하기는 쉽지 않다. 한번 쓴 글을 다시 수정하지 않을 때가 많다.
[단순함을 이루기 위해서는 ‘치열함’이 요구](https://kr.analysisman.com/2020/09/success-intense.html)되듯, 퇴고에도 이런 집착이 필요하다.
**의식적으로 다시 수정하겠다는 의지를 갖고 깎아내지 않는 이상 아마추어에 머물 것이다**.

## Summary

### 스토리텔링
- 아마추어: 최초 발상에 대한 맹신, 어떻게 스토리를 재미있게 보여줄 수 있을까?에 대한 고민만 한다.
- 상업 대중 예술을 할 때 가장 중요한 건 그 컨텐츠가 '**재미**'가 있는가 여야 합니다.
- "내가 보여주고 싶은 이야기"가 중심이 아니다. (작가주의 작품의 경우 예외)
- 명작은 연필이 아니라 지우개에서 나온다.
- TODO 아마추어가 프로로 전향하는 가장 첫 번째 단계는, 최초의 발상에서 진행된 그 스토리, 그 스토리야말로 그 발상에서 진행할 수 있는 가장 재미없는 프로토타입 임을 인정할 것! 

> [!NOTE]
 :: → 윗 부분이 이해안됨</br>다만 첫 발상에 머물게 된다면, </br>즉 맹신하게되면 전개가 어렵게되며 </br>스토리 자체가 개인적인 것이 된다. </br>상대방에게 재미있는 이야기를 들려주듯 군더더기는 들어내어 정리한다.
> - 미켈란젤로의 다비드 상
> - 다비드를 꺼내기 위해 필요없는 부분을 깍아낸다네


## References

- [프로와 아마추어의 차이 / 아마추어의 습관 / 스토리 텔링 / 이야기 강의 / 시나리오 공부 / 시나리오쓰기 / 영화시나리오 / 소설 / 웹툰 / 웹소설 / 스토리 작가 - YouTube](https://youtu.be/mpcQCdBQ79k)
